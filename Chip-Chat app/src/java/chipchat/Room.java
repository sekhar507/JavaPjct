...
public final class Room extends Thread {
	...
	int enterUser(final User user, final String passwd) {
		synchronized (users) {
			...
			// Password검사.
			if (this.passwd != null
				&& (!this.passwd.equalsIgnoreCase(passwd))) {
				return -2;
			}
	...
	/*
	 * Functions relativing to message.
	 */
	/**
	 * Input message.
	 * @param msg Message
	...
	void inputKickOut(final int from, final int to) {
		if (from == host) {
			...
			if (user == null) {
				...
				inputMsg(
					new Msg(Msg.TYPE_KICKOUT, -1, "" + to, user.getUsername()));
			}
		} else {
			inputError(from, "YouNotMaster");
		}
				...
				if (msg != null) {
					...
					synchronized (users) {
						for (Iterator i = users.keySet().iterator();
							...
							User user = (User) users.get(key);
							String smsg =
								lmsg.getString(user.getUserid().intValue());
							if (smsg != null) {
								try {
